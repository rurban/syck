...
void cocoa_syck_error_handler(SyckParser *p, const char *msg) {
  NSLog(@"syck error:%s position:(%d, %d)", msg, p->linect,
        p->cursor - p->lineptr);
}
...
SYMID cocoa_syck_parse_handler(SyckParser *p, SyckNode *n) {
  ...
  case syck_str_kind:
    ...
    if (type_id == NULL || strcmp(type_id, "str") == 0) {
      v = [NSString yamlStringWithUTF8String:n->data.str->ptr
                                      length:n->data.str->len];
    } else if (strcmp(type_id, "null") == 0) {
      v = [NSNull null];
    } else if (strcmp(type_id, "binary") == 0) {
      v = [NSData
          dataWithBase64EncodedString:[NSString
                                          yamlStringWithUTF8String:n->data.str
                                                                       ->ptr
                                                            length:n->data.str->
                                                                   len]];

      NSString *string = [NSString yamlStringWithUTF8String:[v bytes]
                                                     length:[v length]];
      if (string) {
        v = string;
      }
...
/*            else if ( strcmp( type_id, "timestamp#iso8601" ) == 0 )
            {
                v = [NSDate dateWithNaturalLanguageString:[NSString
   stringWithUTF8String:n->data.str->ptr length:n->data.str->len]];
            }
   ...
   stringWithUTF8String:n->data.str->ptr length:n->data.str->len]];
      ...
      v = [NSDate dateWithNaturalLanguageString:
                      [[[NSString alloc]
                          initWithData:[NSData dataWithBytes:n->data.str->ptr
                                                      length:n->data.str->len]
    ...
    for (i = 0; i < n->data.pairs->idx; i++) {
      ...
      if (o2 == @"MERGE") {
        if ([o3 isKindOfClass:[NSDictionary class]])
          ...
          [v yamlPerformSelector:@selector(addEntriesFromDictionary:)
              withEachObjectInArray:o3];
      } else
