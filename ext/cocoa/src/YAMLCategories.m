...
- (NSArray *)yamlData {
  ...
  while (object = [enumerator nextObject]) {
    /*if(!yamlClass(object))
    {
            [array addObject:[YAMLWrapper wrapperWithData:[object yamlData]
    tag:[object class]]];
    }
    else*/
...
- (NSString *)yamlDescriptionWithIndent:(int)indent {
  ...
  while (anObject = [enumerator nextObject]) {
    ...
    if (yamlClass(anObject))
      ...
      tag = [NSString
          stringWithFormat:@"!!%@ ", NSStringFromClass([anObject class])];

    anObject = [anObject toYAML];

    [description appendFormat:@"%s- %@%@%s", strIndent, tag,
                              [anObject yamlDescriptionWithIndent:indent + 2],
                              anObject == last ? "" : "\n"];
  }
...
- (NSDictionary *)yamlData {
  ...
  while (key = [enumerator nextObject]) {
    ...
    /*if(!yamlClass(object))
    {
            [dict setObject:[YAMLWrapper wrapperWithData:[object yamlData]
    tag:[object class]] forKey:key];
    }
    else*/
...
- (NSString *)yamlDescriptionWithIndent:(int)indent {
  ...
  /*if([[allKeys objectAtIndex:0]
  respondsToSelector:@selector(caseInsensitiveCompare:)]) { allKeys = [allKeys
  sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)]; } else { allKeys
  ...
  while (key = [enumerator nextObject]) {
    ...
    if (yamlClass(object))
      ...
      tag = [NSString
          stringWithFormat:@"!!%@ ", NSStringFromClass([object class])];

    object = [object toYAML];

    /*[description appendFormat:@"%s%@: %@%@%s", strIndent,
        [key stringByPaddingToLength:keyLength withString:@" "
       startingAtIndex:0], tag, [object yamlDescriptionWithIndent:indent+2]];*/

    [description appendFormat:@"%s%@: %@%@\n", strIndent, key, tag,
                              [object yamlDescriptionWithIndent:indent + 2]];
  }
  [description
      deleteCharactersInRange:NSMakeRange([description length] - 1, 1)];
...
- (id)yamlDescriptionWithIndent:(int)indent {
  return [[@"!binary |\n"
      stringByAppendingString:[self base64EncodingWithLineLength:72]]
      yamlIndented:indent];
}
