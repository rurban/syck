  ...
  if ([data isKindOfClass:[NSDictionary class]]) {
    if ([data objectForKey:@"z"]) {
      if ([data objectForKey:@"r"]) {
        return [NSColor
            colorWithCalibratedRed:[[data objectForKey:@"r"] floatValue]
                             green:[[data objectForKey:@"g"] floatValue]
                              blue:[[data objectForKey:@"b"] floatValue]
                             alpha:[[data objectForKey:@"a"] floatValue]];
      } else {
        return [NSColor
            colorWithCalibratedWhite:[[data objectForKey:@"w"] floatValue]
                               alpha:[[data objectForKey:@"a"] floatValue]];
      }
    } else {
      if ([data objectForKey:@"r"]) {
        return
            [NSColor colorWithDeviceRed:[[data objectForKey:@"r"] floatValue]
                                  green:[[data objectForKey:@"g"] floatValue]
                                   blue:[[data objectForKey:@"b"] floatValue]
                                  alpha:[[data objectForKey:@"a"] floatValue]];
      } else if ([data objectForKey:@"w"]) {
        return [NSColor
            colorWithDeviceWhite:[[data objectForKey:@"w"] floatValue]
                           alpha:[[data objectForKey:@"a"] floatValue]];
      } else if ([data objectForKey:@"k"]) {
        return
            [NSColor colorWithDeviceCyan:[[data objectForKey:@"c"] floatValue]
                                 magenta:[[data objectForKey:@"m"] floatValue]
                                  yellow:[[data objectForKey:@"y"] floatValue]
                                   black:[[data objectForKey:@"k"] floatValue]
                                   alpha:[[data objectForKey:@"a"] floatValue]];
      } else {
        return [NSColor colorWithCatalogName:[data objectForKey:@"catalog"]
                                   colorName:[data objectForKey:@"name"]];
      }
    }
  }
...
- (id)toYAML {
  ...
  if ([colorSpaceName isEqualToString:NSDeviceRGBColorSpace]) // rgb
  {
    return [NSDictionary
        dictionaryWithObjectsAndKeys:[NSNumber
                                         numberWithFloat:[self redComponent]],
                                     @"r",
                                     [NSNumber
                                         numberWithFloat:[self greenComponent]],
                                     @"g",
                                     [NSNumber
                                         numberWithFloat:[self blueComponent]],
                                     @"b",
                                     [NSNumber
                                         numberWithFloat:[self alphaComponent]],
                                     @"a", NULL];
  } else if ([colorSpaceName
                 isEqualToString:NSCalibratedRGBColorSpace]) // calibrated rgb
  {
    return [NSDictionary
        dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:1], @"z",
                                     [NSNumber
                                         numberWithFloat:[self redComponent]],
                                     @"r",
                                     [NSNumber
  ...
  } else if ([colorSpaceName isEqualToString:NSDeviceWhiteColorSpace]) // white
  {
    return [NSDictionary
        dictionaryWithObjectsAndKeys:[NSNumber
                                         numberWithFloat:[self whiteComponent]],
                                     @"w",
                                     [NSNumber
                                         numberWithFloat:[self alphaComponent]],
                                     @"a", NULL];
  } else if ([colorSpaceName
                 isEqualToString:NSCalibratedWhiteColorSpace]) // calibrated
                                                               // white
  {
    return [NSDictionary
        dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:1], @"z",
  ...
  {
    return [NSDictionary
        dictionaryWithObjectsAndKeys:[self catalogNameComponent], @"catalog",
...
- (id)toYAML {
  ...
  return [NSArray arrayWithObjects:[NSNumber numberWithFloat:aStruct.m11],
...
+ (id)objectWithYAML:(id)data {
  if ([data objectForKey:@"x"] && [data objectForKey:@"w"]) {
    return [NSValue
        valueWithRect:NSMakeRect([[data objectForKey:@"x"] floatValue],
                                 [[data objectForKey:@"y"] floatValue],
                                 [[data objectForKey:@"w"] floatValue],
                                 [[data objectForKey:@"h"] floatValue])];
  } else if ([data objectForKey:@"x"]) {
    return [NSValue
        valueWithPoint:NSMakePoint([[data objectForKey:@"x"] floatValue],
                                   [[data objectForKey:@"y"] floatValue])];
  } else if ([data objectForKey:@"w"]) {
    return [NSValue
        valueWithSize:NSMakeSize([[data objectForKey:@"w"] floatValue],
                                 [[data objectForKey:@"h"] floatValue])];
  } else if ([data objectForKey:@"p"]) {
    return [NSValue
        valueWithRange:NSMakeRange([[data objectForKey:@"p"] floatValue],
                                   [[data objectForKey:@"l"] floatValue])];
  }
...
- (id)toYAML {
  if (strcmp([self objCType], "{_NSPoint=ff}") == 0) {
    ...
    return [NSDictionary
        dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:p.x], @"x",
    ...
    return [NSDictionary
        dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:p.width], @"w",
                                     [NSNumber numberWithFloat:p.height], @"h",
                                     NULL];
  }
  return NULL;
}
