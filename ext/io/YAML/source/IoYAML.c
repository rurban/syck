...
YAML ioDoc(
                    ...
                    docObject("YAML")
                    docDescription("""YAML is a simple text language for sending
lists, maps and objects containing primitive types.""") docCategory("Databases")
                    */
...
IoYAML *IoYAML_rawClone(IoYAML *proto) {
  ...
  IoObject_setDataPointer_(
      self, cpalloc(IoObject_dataPointer(proto), sizeof(IoYAMLData)));
  return self;
}
...
void IoYAML_free(IoYAML *self) { free(IoObject_dataPointer(self)); }
...
static void IoYAML_emitHandler(SyckEmitter *e, st_data_t data) {
  ...
  if (ISNIL(obj)) {
    ...
    syck_emit_scalar(e, "boolean", scalar_none, 0, 0, 0, (char *)bool_s,
                     strlen(bool_s));
  } else if (ISSEQ(obj)) {
    if (!obj) {
    } else
      syck_emit_scalar(e, "string", scalar_none, 0, 0, 0,
                       (char *)IOSEQ_BYTES(obj), IOSEQ_LENGTH(obj));
