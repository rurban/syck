  ...
  case syck_str_kind:
    if (n->type_id == NULL || strcmp(n->type_id, "str") == 0) {
      ...
      o = lua_gettop(bonus->L);
               ...
               strcmp(n->type_id, "float#fix") == 0 ||
               strcmp(n->type_id, "float#exp") == 0) {
      double f;
...
void lua_syck_emitter_handler(SyckEmitter *e, st_data_t data) {
  ...
  case LUA_TBOOLEAN:
    if (lua_toboolean(bonus->L, -1))
      ...
      strcpy(buf, "false");
    syck_emit_scalar(e, "boolean", scalar_none, 0, 0, 0, (char *)buf,
                     strlen(buf));
    break;
  case LUA_TSTRING:
    syck_emit_scalar(e, "string", scalar_none, 0, 0, 0,
                     (char *)lua_tostring(bonus->L, -1),
                     lua_strlen(bonus->L, -1));
    break;
  case LUA_TNUMBER:;
...
static const luaL_Reg sycklib[] = {
    {"load", syck_load}, {"dump", syck_dump}, {NULL, NULL}};
