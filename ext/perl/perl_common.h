...
#undef VERSION /* maybe defined on the command-line */
#include <syck.h>
...
SyckNode *perl_syck_bad_anchor_handler(SyckParser *p, const char *a) {
  SyckNode *badanc = syck_new_map((SYMID)newSVpvn_share("name", 4, 0),
                                  (SYMID)newSVpvn_share(a, strlen(a), 0));
  badanc->type_id = syck_strndup("!perl:YAML::Syck::BadAlias", 26);
  return badanc;
}

void perl_syck_error_handler(SyckParser *p, const char *msg) {
  croak("%s parser (line %d, column %ld): %s", "Syck", p->linect + 1,
        (long)(p->cursor - p->lineptr), msg);
}

void perl_syck_output_handler_pv(SyckEmitter *e, const char *str, long len) {
  struct emitter_xtra *bonus = (struct emitter_xtra *)e->bonus;
  sv_catpvn_nomg(bonus->out.outsv, str, len);
}

void perl_syck_output_handler_mg(SyckEmitter *e, const char *str, long len) {
  struct emitter_xtra *bonus = (struct emitter_xtra *)e->bonus;
  sv_catpvn_mg(bonus->out.outsv, str, len);
}

void perl_syck_output_handler_io(SyckEmitter *e, const char *str, long len) {
  struct emitter_xtra *bonus = (struct emitter_xtra *)e->bonus;
  if (bonus->ioerror) {
    return;
  } else {
