...
#define TRACK_OBJECT(sv)                                                       \
  (av_push(((struct parser_xtra *)p->bonus)->objects, sv))
...
#define SvRV_set(sv, val)                                                      \
  STMT_START { (SvRV(sv) = (val)); }                                           \
  STMT_END
#endif

static const char *is_bad_alias_object(SV *sv) {
  SV *hv, **psv;
...
static void register_bad_alias(SyckParser *p, const char *anchor, SV *sv) {
  HV *map;
  SV **pref_av, *new_rvav;
  AV *rvs;
...
static void resolve_bad_alias(SyckParser *p, const char *anchor, SV *sv) {
  HV *map;
  SV **pref_av, *entity;
  AV *rvs;
  I32 len, i;
    ...
    if (TYPE_IS_NULL(id)) {
      ...
      if (load_blessed) {
        if (lang == NULL || (strEQ(lang, "perl"))) {
          ...
          sv_bless(
              sv, gv_stashpv(form((type == NULL) ? "%s" : "%s::%s", lang, type),
                             TRUE));
        }
      }
      ...
      if (load_blessed) {
        if (strnNE(ref_type, REF_LITERAL, REF_LITERAL_LENGTH + 1)) {
          ...
          if (lang == NULL || (strEQ(lang, "perl"))) {
            ...
            sv_bless(sv, gv_stashpv(
                             form((type == NULL) ? "%s" : "%s::%s", lang, type),
                             TRUE));
          }
        }
      }
    } else if ((id != NULL) &&
               (strEQ(id, "perl/regexp") || strnEQ(id, "perl/regexp:", 12))) {
...
void perl_json_postprocess(SV *sv) {
   ...
   */
  if ((json_quote_char == '\'') && (len > 1) && (*s == '\"') &&
      (*(s + len - 2) == '\"')) {
    *s = '\'';
    *(s + len - 2) = '\'';
  }

  /* 2010-07-20 - TODDR: This for loop doesn't appear to do anything other than
   * shorten the line if it sees [,:] when not in quotes. Even then it appears
   * that the \0 isn't being placed right if that happens. TODO: need test case
   * to prove this does not work as expected.
   */
  for (i = 0; i < len; i++) {
    ch = *(s + i);
    *pos++ = ch;
...
static SV *LoadYAML(char *s) {
  ...
  SV *obj = &PL_sv_undef;
  SV *use_code =
      GvSV(gv_fetchpv(form("%s::UseCode", PACKAGE_NAME), TRUE, SVt_PV));
  SV *load_code =
      GvSV(gv_fetchpv(form("%s::LoadCode", PACKAGE_NAME), TRUE, SVt_PV));
  SV *implicit_typing =
      GvSV(gv_fetchpv(form("%s::ImplicitTyping", PACKAGE_NAME), TRUE, SVt_PV));
  SV *implicit_unicode =
      GvSV(gv_fetchpv(form("%s::ImplicitUnicode", PACKAGE_NAME), TRUE, SVt_PV));
  SV *singlequote =
      GvSV(gv_fetchpv(form("%s::SingleQuote", PACKAGE_NAME), TRUE, SVt_PV));
  SV *load_blessed =
      GvSV(gv_fetchpv(form("%s::LoadBlessed", PACKAGE_NAME), TRUE, SVt_PV));
...
(SyckEmitter *e, SV *sv) {
  ...
  if (e->depth >= e->max_depth) {
    croak("Dumping circular structures is not supported with JSON::Syck, "
          "consider increasing $JSON::Syck::MaxDepth higher then %d.",
          e->max_depth);
  }
#endif
...
#define OBJECT_TAG "tag:!perl:"
  ...
  if (SvROK(sv)) {
    ...
    case SVt_PVHV:
    case SVt_PVCV: {
      /* Arrays, hashes and code values are inlined, and will be wrapped by a
       * ref in the undumping */
      e->indent = 0;
    ...
    case SVt_REGEXP: {
      STRLEN len = sv_len(sv);
      syck_emit_scalar(e, OBJOF("tag:!perl:regexp"), SCALAR_STRING, 0, 0, 0,
                       SvPV_nolen(sv), len);
      syck_emit_end(e);
      break;
    }
    ...
    default: {
      ...
      *tag = '\0';
      syck_emit_item(
          e, (st_data_t)newSVpvn_share(REF_LITERAL, REF_LITERAL_LENGTH, 0));
      syck_emit_item(e, (st_data_t)SvRV(sv));
      syck_emit_end(e);
    }
    }
    ...
    /* emit an undef */
    syck_emit_scalar(e, "str", scalar_plain, 0, 0, 0, NULL_LITERAL,
                     NULL_LITERAL_LENGTH);
  } else if ((ty == SVt_PVMG) && !SvOK(sv)) {
    /* emit an undef (typically pointed from a blesed SvRV) */
    syck_emit_scalar(e, OBJOF("str"), scalar_plain, 0, 0, 0, NULL_LITERAL,
                     NULL_LITERAL_LENGTH);
  } else if (SvPOK(sv)) {
    /* emit a string */
    STRLEN len = sv_len(sv);
 ...
 * javascript) */
    ...
    if (looks_like_number(sv)) {
      if (syck_str_is_unquotable_integer(SvPV_nolen(sv), sv_len(sv))) {
        /* emit an unquoted number only if it's a very basic integer.
         * /^-?[1-9][0-9]*$/ */
        syck_emit_scalar(e, OBJOF("str"), SCALAR_NUMBER, 0, 0, 0,
                         SvPV_nolen(sv), len);
      } else {
        /* Even though it looks like a number, quote it or it won't round trip
         * correctly. */
        syck_emit_scalar(e, OBJOF("str"), SCALAR_QUOTED, 0, 0, 0,
                         SvPV_nolen(sv), len);
      }
    } else
#endif
      ...
      e->style = SCALAR_UTF8;
      syck_emit_scalar(e, OBJOF("str"), SCALAR_STRING, 0, 0, 0, SvPV_nolen(sv),
                       len);
      e->style = old_s;
    }
#ifndef YAML_IS_JSON
      ...
      for (i = 0; i < len; i++) {
        if (*(str + i) & 0x80) {
          ...
          syck_emit_scalar(e, "tag:yaml.org,2002:binary", SCALAR_STRING, 0, 0,
                           0, base64, strlen(base64));
          is_ascii = FALSE;
          break;
        }
      }
    ...
    if (SvIOK(sv) /* original SV was an int */
        && syck_str_is_unquotable_integer(
               str, len)) /* small enough to safely round-trip */
    {
...
#if PERL_VERSION < 8
      syck_emit_scalar(e, OBJOF("tag:!perl:code:"), SCALAR_QUOTED, 0, 0, 0,
                       "{ \"DUMMY\" }", 11);
#else
      if (!dump_code) {
        syck_emit_scalar(e, OBJOF("tag:!perl:code:"), SCALAR_QUOTED, 0, 0, 0,
        ...
        CV *cv = (CV *)sv;
        SV *bdeparse = GvSV(
            gv_fetchpv(form("%s::DeparseObject", PACKAGE_NAME), TRUE, SVt_PV));
        ...
        if (len == 0 || *(SvPV_nolen(text) + reallen - 1) == ';') {
          croak("The result of B::Deparse::coderef2text was empty - maybe "
                "you're trying to serialize an XS function?\n");
        }
        ...
        syck_emit_scalar(e, OBJOF("tag:!perl:code:"), SCALAR_UTF8, 0, 0, 0,
                         SvPV_nolen(text), reallen);
...
(SV *sv, struct emitter_xtra *bonus, SyckOutputHandler output_handler) {
  ...
  SV *headless =
      GvSV(gv_fetchpv(form("%s::Headless", PACKAGE_NAME), TRUE, SVt_PV));
  SV *implicit_binary =
      GvSV(gv_fetchpv(form("%s::ImplicitBinary", PACKAGE_NAME), TRUE, SVt_PV));
  SV *use_code =
      GvSV(gv_fetchpv(form("%s::UseCode", PACKAGE_NAME), TRUE, SVt_PV));
  SV *dump_code =
      GvSV(gv_fetchpv(form("%s::DumpCode", PACKAGE_NAME), TRUE, SVt_PV));
  SV *sortkeys =
      GvSV(gv_fetchpv(form("%s::SortKeys", PACKAGE_NAME), TRUE, SVt_PV));
#ifdef YAML_IS_JSON
  SV *singlequote =
      GvSV(gv_fetchpv(form("%s::SingleQuote", PACKAGE_NAME), TRUE, SVt_PV));
  SV *max_depth =
      GvSV(gv_fetchpv(form("%s::MaxDepth", PACKAGE_NAME), TRUE, SVt_PV));
  json_quote_char = (SvTRUE(singlequote) ? '\'' : '"');
  ...
  if (SvTRUE(use_code) || SvTRUE(dump_code)) {
    SV *bdeparse =
        GvSV(gv_fetchpv(form("%s::DeparseObject", PACKAGE_NAME), TRUE, SVt_PV));

    if (!SvTRUE(bdeparse)) {
      eval_pv(form("local $@; require B::Deparse; $%s::DeparseObject = "
                   "B::Deparse->new",
                   PACKAGE_NAME),
              1);
    }
