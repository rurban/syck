...
NEED_grok_numeric_radix_GLOBAL grok_oct()                NEED_grok_oct
NEED_grok_oct_GLOBAL load_module()             NEED_load_module
NEED_load_module_GLOBAL mess()                    NEED_mess NEED_mess_GLOBAL
    ...
    pv_display()              NEED_pv_display NEED_pv_display_GLOBAL pv_escape()
NEED_pv_escape               NEED_pv_escape_GLOBAL pv_pretty() NEED_pv_pretty
NEED_pv_pretty_GLOBAL sv_catpvf_mg()            NEED_sv_catpvf_mg
NEED_sv_catpvf_mg_GLOBAL sv_catpvf_mg_nocontext()  NEED_sv_catpvf_mg_nocontext
NEED_sv_catpvf_mg_nocontext_GLOBAL sv_setpvf_mg()            NEED_sv_setpvf_mg
NEED_sv_setpvf_mg_GLOBAL sv_setpvf_mg_nocontext()  NEED_sv_setpvf_mg_nocontext
NEED_sv_setpvf_mg_nocontext_GLOBAL sv_unmagicext()           NEED_sv_unmagicext
NEED_sv_unmagicext_GLOBAL utf8_to_uvchr_buf()       NEED_utf8_to_uvchr_buf
NEED_utf8_to_uvchr_buf_GLOBAL vload_module()            NEED_vload_module
NEED_vload_module_GLOBAL vmess()                   NEED_vmess NEED_vmess_GLOBAL
...
{ my @deps = map { s/\s+//g; $_ } split /,/, $3; my $d; for $d (map { s/\s+//g;
$_ } split /,/, $1) { push @{$depends{$d}}, @deps;
    }
  }
...
if (exists $opt{'api-info'}) {
  ...
  for $f (sort dictionary_order keys %API) {
    ...
    if ($base && ! $API{$f}{inaccessible} && ! $API{$f}{core_only}) {
        ...
        if ($API{$f}{unverified}) {
            print "\nThis information is based on inspection of the source
code", " and has not been\n", "verified by successful compilation.";
     ...
     if ($API{$f}{provided} || $todo) {
        print "\nThis is only supported by $ppport, and NOT by perl versions
going forward.\n" unless $base; if ($todo) { if (! $base || $todo < $base) { my
$additionally = ""; $additionally .= " additionally" if $base; print
"$ppport$additionally provides support at least back to perl-",
                    format_version($todo),
                    ".\n";
            }
        }
            ...
            if (exists $depends{$f}) {
                my $max = 0;
                for (@{$depends{$f}}) {
                    $max = int_parse_version($API{$_}{todo}) if $API{$_}{todo}
&& $API{$_}{todo} > $max; # XXX What to assume unspecified values are?  This
effectively makes them MIN_PERL
                }
                $todo = $max if $max;
            }
            print "\n$ppport provides support for this, but ironically, does
not", " currently know,\n", "for this report, the minimum version it supports
for this"; if ($API{$f}{undocumented}) { print " and many things\n", "it
provides that are implemented as macros and aren't", " documented.  You can\n",
    ...
    if ($base || ! $API{$f}{ppport_fnc}) {
      my $email = "Send email to perl5-porters\@perl.org if you need to have
this functionality.\n"; if ($API{$f}{inaccessible}) { print "\nThis is not part
of the public API, and may not even be accessible to XS code.\n"; $info++;
      }
      elsif ($API{$f}{core_only}) {
        print "\nThis is not part of the public API, and should not be used by
XS code.\n"; $info++;
      }
      elsif ($API{$f}{deprecated}) {
        print "\nThis is deprecated and should not be used.  Convert existing
uses.\n"; $info++;
      }
      elsif ($API{$f}{experimental}) {
        print "\nThe API for this is unstable and should not be used by XS
code.\n", $email; $info++;
      }
      elsif ($API{$f}{undocumented}) {
        print "\nSince this is undocumented, the API should be considered
unstable.\n"; if ($API{$f}{provided}) { print "Consider bringing this up on the
list: perl5-porters\@perl.org.\n";
        }
        else {
            print "It may be that this is not intended for XS use, or it may
...
for $filename (@files) {
  ...
  for $func (keys %API) {
    ...
    if ($c =~ /\b(?:Perl_)?($match)\b/) {
      $file{uses_replace}{$1}++ if exists $revreplace{$func} && $1 eq
$revreplace{$func}; $file{uses_Perl}{$func}++ if $c =~ /\bPerl_$func\b/; if
(exists $API{$func}{provided}) { $file{uses_provided}{$func}++; if ( ! exists
$API{$func}{base}
  ...
  for $func (sort dictionary_order keys %{$file{uses_provided}}) {
    if ($file{uses}{$func}) {
      if (exists $file{uses_deps}{$func}) {
        diag("Uses $func, which depends on ", join(', ',
@{$file{uses_deps}{$func}}));
      }
      else {
        diag("Uses $func");
  ...
  for $func (sort dictionary_order keys %{$file{needed_global}}) {
    ...
    if (not exists $global{uses}{$func}) {
      ...
      if ($file{needs}{$func} eq 'extern') {
        $message = "No need to define NEED_${func}_GLOBAL when already needed
globally";
      }
      elsif ($file{needs}{$func} eq 'static') {
        $message = "No need to define NEED_${func}_GLOBAL when only used in this
file";
      }
    }
    if ($message) {
      diag($message);
      $file{changes} += ($c =~
s/^$HS*#$HS*define$HS+NEED_${func}_GLOBAL\b.*$LF//mg);
    }
  }
  ...
  if ($file{needs_inc_ppport}) {
    ...
    for $func (sort dictionary_order keys %{$file{needs}}) {
      ...
      unless (exists $file{"needed_$type"}{$func}) {
        if ($type eq 'global') {
          diag("Files [@{$global{needs}{$func}}] need $func, adding global
request");
        }
        else {
  ...
  if ($file{changes}) {
    if (exists $opt{copy}) {
      ...
      if (-e $newfile) {
        error("'$newfile' already exists, refusing to write copy of
'$filename'");
      }
      else {
        local *F;
...
/^=head\d$HS+SYNOPSIS\s*^(.*?)\s*^=/ms; my %M = ( 'I' => '*' ); $usage =~
s/^\s*perl\s+\S+/$^X $0/; $usage =~ s/([A-Z])<([^>]+)>/$M{$1}$2$M{$1}/g;
...
#define PERL_BCDVERSION                                                        \
  ((D_PPP_DEC2BCD(PERL_REVISION) << 24) |                                      \
   (D_PPP_DEC2BCD(PERL_VERSION) << 12) | D_PPP_DEC2BCD(PERL_SUBVERSION))
...
#define D_PPP_my_PL_parser_var(var)                                            \
  ((PL_parser                                                                  \
        ? PL_parser                                                            \
        : (croak("panic: PL_parser == NULL in %s:%d", __FILE__, __LINE__),     \
           (yy_parser *)NULL))                                                 \
       ->var)
#else
...
#define D_PPP_my_PL_parser_var(var)                                            \
  ((PL_parser ? PL_parser                                                      \
              : (D_PPP_parser_dummy_warning(var) & DPPP_(dummy_PL_parser)))    \
       ->var)
 ...
 * PL_bufptr, PL_bufend, PL_lex_state, PL_lex_stuff, PL_tokenbuf Do not use this
 * variable unless you know exactly what you're doing. It is internal to the
 * perl parser and may change or even be removed in the future. As of
 * perl 5.9.5, you have to check for (PL_parser != NULL) for this variable to
 * have any effect. An always non-NULL PL_parser dummy is provided for earlier
 * perl versions.
...
#ifndef SvRX
#define SvRX(rv)                                                               \
  (SvROK((rv)) ? (SvMAGICAL(SvRV((rv)))                                        \
                      ? (mg_find(SvRV((rv)), PERL_MAGIC_qr)                    \
...
#ifndef isCNTRL_L1
#define isCNTRL_L1(c)                                                          \
  ((WIDEST_UTYPE)(c) < ' ' || (WIDEST_UTYPE)(c) == D_PPP_OUTLIER_CONTROL)
#endif

                            )
                            /* The ordering of the tests in this and isUPPER are
                             * to exclude most characters early */
#ifndef isLOWER
#define isLOWER(c)                                 \
                              ((c) >= 'a' && (c) <= 'z' &&                     \
                               ((c) <= 'i' || ((c) >= 'j' && (c) <= 'r') ||    \
...
#if (PERL_BCDVERSION <                             \
                                 0x5008000) /* earlier perls omitted DEL */
#undef isCNTRL
#endif
...
#if (PERL_BCDVERSION <                             \
                                 0x5017008) /* earlier perls didn't include    \
                                               PILCROW, SECTION SIGN */
#undef isPUNCT_L1
#endif

#if (PERL_BCDVERSION <                             \
                                 0x5013007) /* khw didn't investigate why this \
                                               failed */
#undef isALNUMC_L1
#endif

#if (PERL_BCDVERSION <                             \
                                 0x5020000) /* earlier perls didn't include \v \
                                             */
#undef isSPACE
#undef isSPACE_A
#undef isSPACE_L1
...
#endif /* Below are definitions common to EBCDIC   \
                                      and ASCII */
#ifndef isASCII_L1
...
#define isIDFIRST_L1(c)                            \
                              (isALPHA_L1(c) || NATIVE_TO_LATIN1(c) == '_')
#endif
...
#ifndef isPRINT_L1
#define isPRINT_L1(c)                              \
                              ((WIDEST_UTYPE)(c) < 256 && !isCNTRL_L1(c))
#endif
...
#define isXDIGIT_A(c) isXDIGIT(c)
                            ...
                            /* Until we figure out how to support this in older
                             * perls... */
#if (PERL_BCDVERSION >= 0x5008000)
#ifndef HeUTF8
#define HeUTF8(he)                                 \
...
#define vnewSVpvf(pat, args)                       \
                              ({                                               \
                                SV *_sv = newSV(0);                            \
                                sv_vsetpvfn(_sv, (pat), strlen((pat)), (args), \
                                            Null(SV **), 0, Null(bool *));     \
                                _sv;                                           \
                              })
#else
#define vnewSVpvf(pat, args)                       \
                              ((PL_Sv = newSV(0)),                             \
                               sv_vsetpvfn(PL_Sv, (pat), strlen((pat)),        \
                                           (args), Null(SV **), 0,             \
                                           Null(bool *)),                      \
                               PL_Sv)
#endif
...
#define sv_vcatpvf(sv, pat, args)                  \
                              sv_vcatpvfn(sv, pat, strlen(pat), args,          \
                                          Null(SV **), 0, Null(bool *))
#endif
...
#define sv_vsetpvf(sv, pat, args)                  \
                              sv_vsetpvfn(sv, pat, strlen(pat), args,          \
                                          Null(SV **), 0, Null(bool *))
#endif

#if (PERL_BCDVERSION >= 0x5004000) &&              \
                                !defined(sv_catpvf_mg)
...
#define Perl_sv_catpvf_mg DPPP_(my_sv_catpvf_mg)
                                ...
                                void
                                DPPP_(my_sv_catpvf_mg)(pTHX_ SV *const sv,
                            ...
                            static void DPPP_(my_sv_catpvf_mg_nocontext)(
                                SV *const sv, const char *const pat, ...);
                            static
#else
                            extern void DPPP_(my_sv_catpvf_mg_nocontext)(
                                SV *const sv, const char *const pat, ...);
#endif
...
#define sv_catpvf_mg_nocontext                     \
                              DPPP_(my_sv_catpvf_mg_nocontext)
#define Perl_sv_catpvf_mg_nocontext                \
                              DPPP_(my_sv_catpvf_mg_nocontext)

                                void DPPP_(my_sv_catpvf_mg_nocontext)(
                                    SV *const sv, const char *const pat, ...) {
                              dTHX;
                              va_list args;
                              va_start(args, pat);
...
#if (PERL_BCDVERSION >= 0x5004000) &&              \
                                !defined(sv_setpvf_mg)
#if defined(NEED_sv_setpvf_mg)
                            static void DPPP_(my_sv_setpvf_mg)(
                                pTHX_ SV *const sv, const char *const pat, ...);
                            static
#else
                            extern void DPPP_(my_sv_setpvf_mg)(
                                pTHX_ SV *const sv, const char *const pat, ...);
#endif
...
#define Perl_sv_setpvf_mg DPPP_(my_sv_setpvf_mg)
                                ...
                                void
                                DPPP_(my_sv_setpvf_mg)(pTHX_ SV *const sv,
                                                       const char *const pat,
                                                       ...) {
                              va_list args;
                              va_start(args, pat);
                            ...
                            static void DPPP_(my_sv_setpvf_mg_nocontext)(
                                SV *const sv, const char *const pat, ...);
                            static
#else
                            extern void DPPP_(my_sv_setpvf_mg_nocontext)(
                                SV *const sv, const char *const pat, ...);
#endif
...
#define sv_vsetpvf_mg(sv, pat, args)               \
                            ...
                            /* Hint: sv_2pv_nolen
                             * Use the SvPV_nolen() or SvPV_nolen_const() macros
                             * instead of sv_2pv_nolen().
                             */
#ifndef sv_2pv_nolen
#define sv_2pv_nolen(sv) SvPV_nolen(sv)
                            ...
                            /* Hint: SvPVbyte
                             * Does not work in perl-5.6.1, ppport.h implements
                             * a version borrowed from perl-5.7.3.
                             */

#if (PERL_BCDVERSION < 0x5007000)
#ifndef sv_2pvbyte
#define sv_2pvbyte(sv, lp)                         \
...
#define sv_2pvbyte_nolen(sv) sv_2pv_nolen(sv)
                            ...
                            /* Hint: sv_pvn_force
                             * Always use the SvPV_force() macro instead of
                             * sv_pvn_force().
                             */

                            /* Replace sv_pvn_force with SvPV_force */

                            /* If these are undefined, they're not handled by
                             * the core anyway */
...
#define SvPV_flags_const(sv, lp, flags)            \
                              ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK            \
                                   ...
                                   : (const char *)sv_2pv_flags(               \
                                         sv, &lp, flags | SV_CONST_RETURN))
#endif
...
#define SvPV_force_nolen(sv)                       \
                              SvPV_force_flags_nolen(sv, SV_GMAGIC)
#endif
...
#define SvPV_force_flags_nolen(sv, flags)          \
                              ((SvFLAGS(sv) & (SVf_POK | SVf_THINKFIRST)) ==   \
                                   ...
                                   ? SvPVX(sv)                                 \
                                   : sv_pvn_force_flags(                       \
                                         sv, D_PPP_SVPV_NOLEN_LP_ARG, flags))
#endif
...
#define SvPV_force_flags_mutable(sv, lp, flags)    \
                              ((SvFLAGS(sv) & (SVf_POK | SVf_THINKFIRST)) ==   \
                                   ...
                                   ? ((lp = SvCUR(sv)), SvPVX_mutable(sv))     \
                                   : sv_pvn_force_flags(                       \
                                         sv, &lp, flags | SV_MUTABLE_RETURN))
#endif
#ifndef SvPV_nolen
#define SvPV_nolen(sv)                             \
                              ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK            \
                                   ? SvPVX(sv)                                 \
                                   : sv_2pv_flags(sv, D_PPP_SVPV_NOLEN_LP_ARG, \
                                                  SV_GMAGIC))
#endif
...
#define ckWARN(a) PL_dowarn
                            ...
                            static void DPPP_(my_warner)(U32 err,
                                                         const char *pat, ...);
                            static
#else
                            extern void DPPP_(my_warner)(U32 err,
                                                         const char *pat, ...);
#endif

#if defined(NEED_warner) ||                        \
                                defined(NEED_warner_GLOBAL)

#define Perl_warner DPPP_(my_warner)

                                void DPPP_(my_warner)(U32 err, const char *pat,
                                                      ...) {
                              SV *sv;
                              va_list args;
...
#define XPUSHu(u)                                  \
                            ...
                            static Size_t DPPP_(my_my_strnlen)(const char *str,
                                                               Size_t maxlen);
                            static
#else
                            extern Size_t DPPP_(my_my_strnlen)(const char *str,
                                                               Size_t maxlen);
#endif

#if defined(NEED_my_strnlen) ||                    \
                                defined(NEED_my_strnlen_GLOBAL)
...
#define NEED_vmess
                                ...
                                ((PERL_BCDVERSION >= 0x5006001) &&             \
                                 (PERL_BCDVERSION < 0x5007000))
#if ((PERL_BCDVERSION >= 0x5008000) &&             \
                                 (PERL_BCDVERSION < 0x5008009)) ||             \
                                ((PERL_BCDVERSION >= 0x5009000) &&             \
...
#define warn_sv(sv) warn("%s", SvPV_nolen(sv))
                            ...
                            static SV *DPPP_(my_vmess)(pTHX_ const char *pat,
                                                       va_list *args);
                            static
#else
                            extern SV *DPPP_(my_vmess)(pTHX_ const char *pat,
                                                       va_list *args);
#endif

#if defined(NEED_vmess) ||                         \
                                defined(NEED_vmess_GLOBAL)
...
#define Perl_vmess DPPP_(my_vmess)
                                ...
                                SV *DPPP_(my_vmess)(pTHX_ const char *pat,
                                                    va_list *args) {
                              mess(pat, args);
                              return PL_mess_sv;
                            }
...
#if (PERL_BCDVERSION < 0x5006000) &&               \
                                (PERL_BCDVERSION >= 0x5004000)
#undef mess
#endif

#if !defined(mess_nocontext) &&                    \
                                !defined(Perl_mess_nocontext) &&               \
                                (PERL_BCDVERSION >= 0x5004000)
...
#define Perl_mess_nocontext DPPP_(my_mess_nocontext)
                                ...
                                SV *DPPP_(my_mess_nocontext)(const char *pat,
                                                             ...) {
                              dTHX;
                              SV *sv;
                              va_list args;
...
#define mess Perl_mess_nocontext
                            ...
                            static SV *DPPP_(my_mess_sv)(pTHX_ SV *basemsg,
                                                         bool consume);
                            static
#else
                            extern SV *DPPP_(my_mess_sv)(pTHX_ SV *basemsg,
                                                         bool consume);
#endif

#if defined(NEED_mess_sv) ||                       \
                                defined(NEED_mess_sv_GLOBAL)
...
#define Perl_mess_sv DPPP_(my_mess_sv)
                                ...
                                SV *DPPP_(my_mess_sv)(pTHX_ SV *basemsg,
                                                      bool consume) {
                              SV *tmp;
                              SV *ret;

                              if (SvPOK(basemsg) && SvCUR(basemsg) &&
                                  *(SvEND(basemsg) - 1) == '\n') {
                                if (consume)
                                  return basemsg;
                                ret = mess("");
...
#define croak_memory_wrap()                        \
                            ...
                            static void DPPP_(my_croak_xs_usage)(
                                const CV *const cv, const char *const params);
                            static
#else
                            extern void DPPP_(my_croak_xs_usage)(
                                const CV *const cv, const char *const params);
#endif
...
#define Perl_croak_xs_usage DPPP_(my_croak_xs_usage)
                              ...
                              if (gv) {
                                ...
                                const char *const hvname =
                                    stash ? HvNAME(stash) : NULL;

                                if (hvname)
                                  croak("Usage: %s::%s(%s)", hvname, gvname,
                                        params);
                                else
                                  croak("Usage: %s(%s)", gvname, params);
                              } else {
                                /* Pants. I don't think that it should be
                                 * possible to get here. */
                                croak("Usage: CODE(0x%" UVxf ")(%s)",
                                      PTR2UV(cv), params);
                              }
                            }
#endif
...
#define call_sv(sv, flags)                         \
                              ((flags) & G_METHOD                              \
                                   ? Perl_call_method(                         \
                                         aTHX_(char *) SvPV_nolen_const(sv),   \
                                         (flags) & ~G_METHOD)                  \
...
#define eval_pv(p, croak_on_error)                 \
                            ...
                            static SV *DPPP_(my_eval_pv)(const char *p,
                                                         I32 croak_on_error);
                            static
#else
                            extern SV *DPPP_(my_eval_pv)(const char *p,
                                                         I32 croak_on_error);
#endif

#if defined(NEED_eval_pv) ||                       \
                                defined(NEED_eval_pv_GLOBAL)
...
#if !defined(vload_module) &&                      \
                                defined(start_subparse)
#if defined(NEED_vload_module)
                            static void DPPP_(my_vload_module)(U32 flags,
                                                               SV *name,
                                                               SV *ver,
                                                               va_list *args);
                            static
#else
                            extern void DPPP_(my_vload_module)(U32 flags,
                                                               SV *name,
                                                               SV *ver,
                                                               va_list *args);
#endif
...
#define Perl_vload_module DPPP_(my_vload_module)
                                ...
                                void DPPP_(my_vload_module)(U32 flags, SV *name,
                                                            SV *ver,
                                                            va_list *args) {
                              dTHR;
                              dVAR;
                              ...
                              /* 5.005 has a somewhat hacky force_normal that
                                 doesn't croak on SvREADONLY() if PL_compling is
                                 true. Current perls take care in ck_require()
                                 to correctly turn off SvREADONLY before calling
                                 force_normal_flags(). This seems a better fix
                                 than fudging PL_compling
                               */
                            ...
                            static void DPPP_(my_load_module)(U32 flags,
                                                              SV *name, SV *ver,
                                                              ...);
                            static
#else
                            extern void DPPP_(my_load_module)(U32 flags,
                                                              SV *name, SV *ver,
                                                              ...);
#endif

#if defined(NEED_load_module) ||                   \
                                defined(NEED_load_module_GLOBAL)
...
#define Perl_load_module DPPP_(my_load_module)
                                ...
                                void DPPP_(my_load_module)(U32 flags, SV *name,
                                                           SV *ver, ...) {
                              va_list args;
                              va_start(args, ver);
...
#define newRV_noinc(sv)                            \
                              ((PL_Sv = (SV *)newRV((sv))),                    \
                               SvREFCNT_dec((sv)), PL_Sv)
#endif
#endif

                            /*
                             * Boilerplate macros for initializing and accessing
                             * interpreter-local data from C.  All statics in
                             * extensions should be reworked to use this, if you
                             * want to make the extension thread-safe.  See
                             * ext/re/re.xs for an example of the use of these
                             * macros.
                             *
                             * Code that uses these macros is responsible for
                             * the following:
                             * 1. #define MY_CXT_KEY to a unique string, e.g.
                             * "DynaLoader_guts"
                             * 2. Declare a typedef named my_cxt_t that is a
                             * structure that contains all the data that needs
                             * to be interpreter-local.
                             * 3. Use the START_MY_CXT macro after the
                             * declaration of my_cxt_t.
                             * 4. Use the MY_CXT_INIT macro such that it is
                             * called exactly once (typically put in the BOOT:
                             * section).
                             * 5. Use the members of the my_cxt_t structure
                             * everywhere as MY_CXT.member.
                             * 6. Use the dMY_CXT macro (a declaration) in all
                             * the functions that access MY_CXT.
                             */

#if defined(MULTIPLICITY) ||                       \
                                defined(PERL_OBJECT) || defined(PERL_CAPI) ||  \
                                defined(PERL_IMPLICIT_CONTEXT)

#ifndef START_MY_CXT

                            /* This must appear in all extensions that define a
                             * my_cxt_t structure, right after the definition
                             * (i.e. at file scope).  The non-threads case below
                             * uses it to declare the data as static. */
#define START_MY_CXT

#if (PERL_BCDVERSION < 0x5004068)
                            /* Fetches the SV that keeps the per-interpreter
                             * data. */
#define dMY_CXT_SV                                 \
...
#define dMY_CXT_SV                                 \
                              SV *my_cxt_sv =                                  \
                                  *hv_fetch(PL_modglobal, MY_CXT_KEY,          \
                                            sizeof(MY_CXT_KEY) - 1, TRUE)
#endif /* < perl5.004_68 */

                            /* This declaration should be used within all
                             * functions that use the interpreter-local data. */
#define dMY_CXT                                    \
                              dMY_CXT_SV;                                      \
                              my_cxt_t *my_cxtp =                              \
                             ...
                             * (We allocate my_cxtp in a Perl SV so that it will
                             * be released when the interpreter goes away.) */
#define MY_CXT_INIT                                \
                              dMY_CXT_SV;                                      \
                              /* newSV() allocates one more than needed */     \
                              my_cxt_t *my_cxtp = (my_cxt_t *)SvPVX(           \
                                  newSV(sizeof(my_cxt_t) - 1));                \
                              Zero(my_cxtp, 1, my_cxt_t);                      \
                              sv_setuv(my_cxt_sv, PTR2UV(my_cxtp))

                            /* This macro must be used to access members of the
                             * my_cxt_t structure. e.g. MYCXT.some_data */
#define MY_CXT (*my_cxtp)

                            /* Judicious use of these macros can reduce the
                             * number of times dMY_CXT is used.  Use is similar
                             * to pTHX, aTHX etc. */
...
#define MY_CXT_CLONE                               \
                              ...
                              my_cxt_t *my_cxtp = (my_cxt_t *)SvPVX(           \
                                  newSV(sizeof(my_cxt_t) - 1));                \
                              Copy(INT2PTR(my_cxt_t *, SvUV(my_cxt_sv)),       \
                                   my_cxtp, 1, my_cxt_t);                      \
                              sv_setuv(my_cxt_sv, PTR2UV(my_cxtp))
#endif
...
#else
#define newSV_type(t)                              \
                              ((PL_Sv = newSV(0)), sv_upgrade(PL_Sv, (t)),     \
                               PL_Sv)
#endif
#endif
...
#define newSVpvn_flags(s, len, flags)              \
                              ({                                               \
                                SV *_sv =                                      \
                                    newSVpvn(D_PPP_CONSTPV_ARG((s)), (len));   \
...
#define newSVpvn_flags(s, len, flags)              \
                              ((PL_Sv =                                        \
                                    newSVpvn(D_PPP_CONSTPV_ARG((s)), (len))),  \
                               SvFLAGS(PL_Sv) |= ((flags) & SVf_UTF8),         \
                               (((flags) & SVs_TEMP) ? sv_2mortal(PL_Sv)       \
                                                     : PL_Sv))
#endif
#endif

#if ((PERL_BCDVERSION >= 0x5007003) &&             \
                                 (PERL_BCDVERSION < 0x5008007)) ||             \
                                ((PERL_BCDVERSION >= 0x5009000) &&             \
...
#define sv_setsv_flags(dstr, sstr, flags)          \
                              STMT_START {                                     \
                                if (((flags) & SV_NOSTEAL) &&                  \
                                    (SvFLAGS((sstr)) & SVs_TEMP)) {            \
                                  SvTEMP_off((sstr));                          \
                                  Perl_sv_setsv_flags(aTHX_(dstr), (sstr),     \
                                                      (flags) & ~SV_NOSTEAL);  \
                                  SvTEMP_on((sstr));                           \
                                } else {                                       \
                                  Perl_sv_setsv_flags(aTHX_(dstr), (sstr),     \
...
#define newSVsv_flags(sv, flags)                   \
                              ((PL_Sv = newSV(0)),                             \
                               sv_setsv_flags(PL_Sv, (sv), (flags)), PL_Sv)
#endif
...
#define sv_mortalcopy_flags(sv, flags)             \
                              Perl_sv_mortalcopy_flags(aTHX_(sv), (flags))
#endif
...
#define SvUV_set(sv, val)                          \
                            ...
                            /* Hint: newSVpvn_share
                             * The SVs created by this function only mimic the
                             * behaviour of shared PVs without really being
                             * shared. Only use if you know what you're doing.
                             */
                            ...
                            static SV *
                                DPPP_(my_newSVpvn_share)(pTHX_ const char *s,
                                                         I32 len, U32 hash);
                            static
#else
                            extern SV *
...
#if (PERL_BCDVERSION >= 0x5009002) &&              \
                                (PERL_BCDVERSION <=                            \
                                 0x5009003) /* 5.9.2 and 5.9.3 ignore the      \
                                               length param */
...
#define gv_fetchpvn_flags(name, len, flags,        \
                                                      sv_type)                 \
                              gv_fetchpv(                                      \
                                  SvPVX(sv_2mortal(newSVpvn((name), (len)))),  \
                                  ((flags) & GV_NOADD_MASK) ? FALSE : TRUE,    \
                                  (I32)(sv_type))
#endif
#ifndef GvSVn
...
#define gv_init_pvn(gv, stash, ptr, len, flags)    \
                              gv_init(gv, stash, ptr, len,                     \
                                      flags &GV_ADDMULTI ? TRUE : FALSE)
#endif

                            /* concatenating with "" ensures that only literal
                             * strings are accepted as argument note that
                             * STR_WITH_LEN() can't be used as argument to
                             * macros or functions that under some
                             * configurations might be macros
                             */
#ifndef STR_WITH_LEN
...
#define gv_fetchpvs(name, flags, svt)              \
                              gv_fetchpvn_flags(name "", sizeof(name) - 1,     \
                                                flags, svt)
#endif
...
#define SvUV_nomg(sv)                              \
                              (!SvGMAGICAL((sv)) ? SvUV((sv))                  \
                                                 : SvUVx(sv_mortalcopy_flags(  \
                                                       (sv), SV_NOSTEAL)))
#endif

#ifndef SvNV_nomg
#define SvNV_nomg(sv)                              \
                              (!SvGMAGICAL((sv)) ? SvNV((sv))                  \
                                                 : SvNVx(sv_mortalcopy_flags(  \
                                                       (sv), SV_NOSTEAL)))
#endif
...
#define SvTRUE_nomg(sv)                            \
                              (!SvGMAGICAL((sv))                               \
                                   ? SvTRUE((sv))                              \
                                   : SvTRUEx(sv_mortalcopy_flags((sv),         \
                                                                 SV_NOSTEAL)))
#endif
...
#define SvVSTRING_mg(sv)                           \
                             ...
                             * This is a compatibility function that is only
                             * available with Devel::PPPort. It is NOT in the
                             * perl core. Its purpose is to mimic the 5.8.0
                             * behaviour of sv_magic() when it is being passed a
                             * name pointer with namlen == 0. In that case,
                             * perl 5.8.0 and later store the pointer, not a
                             * copy of it. The compatibility can be provided
                             * back to perl 5.004. With earlier versions, the
...
#define sv_magic_portable(sv, obj, how, name,      \
                                                      namlen)                  \
                              STMT_START {                                     \
                                SV *SvMp_sv = (sv);                            \
                                ...
                                if (SvMp_name && SvMp_namlen == 0) {           \
                                  ...
                                  mg = SvMAGIC(SvMp_sv);                       \
                                  mg->mg_len =                                 \
                                      -42; /* XXX: this is the tricky part */  \
                                  mg->mg_ptr = SvMp_name;                      \
                                } else {                                       \
                                  sv_magic(SvMp_sv, obj, how, SvMp_name,       \
                                           SvMp_namlen);                       \
                                }                                              \
                              }                                                \
                              STMT_END
...
#define sv_magic_portable(a, b, c, d, e)           \
                              sv_magic(a, b, c, d, e)
                            ...
                            static MAGIC *
                                DPPP_(my_mg_findext)(const SV *sv, int type,
                                                     const MGVTBL *vtbl);
                            static
#else
                            extern MAGIC *
...
#if defined(NEED_mg_findext) ||                    \
                                defined(NEED_mg_findext_GLOBAL)
...
#define Perl_mg_findext DPPP_(my_mg_findext)
                            ...
                            static int
                                DPPP_(my_sv_unmagicext)(pTHX_ SV *const sv,
                                                        const int type,
                                                        MGVTBL *vtbl);
                            static
#else
...
#define Perl_sv_unmagicext DPPP_(my_sv_unmagicext)
                                  ...
                                  if (mg->mg_ptr &&
                                    ...
                                    if (mg->mg_len > 0)
                                      Safefree(mg->mg_ptr);
                                             ...
                                             HEf_SVKEY) /* Questionable on older
                                                           perls... */
                              ...
                              if (SvMAGIC(sv)) {
                                if (SvMAGICAL(sv)) /* if we're under save_magic,
                                                      wait for restore_magic; */
                                  mg_magical(
                                      sv); /*    else fix the flags now */
                              } else {
                                SvMAGICAL_off(sv);
                                SvFLAGS(sv) |=
                                    (SvFLAGS(sv) &
...
#ifndef CopFILEGV
#define CopFILEGV(c)                               \
                              (CopFILE(c) ? gv_fetchfile(CopFILE(c)) : Nullgv)
#endif

#ifndef CopFILE_set
#define CopFILE_set(c, pv)                         \
                              ((c)->cop_file = savepv(pv))
#endif

#ifndef CopFILESV
#define CopFILESV(c)                               \
                              (CopFILE(c) ? GvSV(gv_fetchfile(CopFILE(c)))     \
                                          : Nullsv)
#endif

#ifndef CopFILEAV
#define CopFILEAV(c)                               \
                              (CopFILE(c) ? GvAV(gv_fetchfile(CopFILE(c)))     \
                                          : Nullav)
#endif
...
#define CopSTASHPV_set(c, pv)                      \
                              ((c)->cop_stashpv = ((pv) ? savepv(pv) : Nullch))
#endif

#ifndef CopSTASH
#define CopSTASH(c)                                \
                              (CopSTASHPV(c)                                   \
                                   ? gv_stashpv(CopSTASHPV(c), GV_ADD)         \
                                   : Nullhv)
#endif

#ifndef CopSTASH_set
#define CopSTASH_set(c, hv)                        \
                              CopSTASHPV_set(c, (hv) ? HvNAME(hv) : Nullch)
#endif
...
#define CopSTASH_eq(c, hv) (CopSTASH(c) == (hv))
                                ...
                                defined(NEED_caller_cx_GLOBAL)
                            static I32
                            DPPP_dopoptosub_at(const PERL_CONTEXT *cxstk,
                                               I32 startingblock) {
                              I32 i;

                              for (i = startingblock; i >= 0; i--) {
                                register const PERL_CONTEXT *const cx =
                                    &cxstk[i];
                                switch (CxTYPE(cx)) {
                                default:
                                  continue;
                            ...
                            static const PERL_CONTEXT *
                                DPPP_(my_caller_cx)(pTHX_ I32 level,
                                                    const PERL_CONTEXT **dbcxp);
                            static
#else
                            extern const PERL_CONTEXT *
                                DPPP_(my_caller_cx)(pTHX_ I32 level,
                                                    const PERL_CONTEXT **dbcxp);
#endif
...
#define caller_cx(a, b)                            \
                              DPPP_(my_caller_cx)(aTHX_ a, b)
#define Perl_caller_cx DPPP_(my_caller_cx)

                                const PERL_CONTEXT *DPPP_(my_caller_cx)(
                                    pTHX_ I32 level,
                                    const PERL_CONTEXT **dbcxp) {
                              register I32 cxix =
                                  DPPP_dopoptosub_at(cxstack, cxstack_ix);
                              register const PERL_CONTEXT *cx;
                              register const PERL_CONTEXT *ccstack = cxstack;
                              const PERL_SI *top_si = PL_curstackinfo;

                              for (;;) {
                                /* we may be in a higher stacklevel, so dig down
                                 * deeper */
                                while (cxix < 0 &&
                                       top_si->si_type != PERLSI_MAIN) {
                                  top_si = top_si->si_prev;
                                  ccstack = top_si->si_cxstack;
                                  cxix = DPPP_dopoptosub_at(ccstack,
                                                            top_si->si_cxix);
                                }
                                if (cxix < 0)
                                  return NULL;
                                /* caller() should not report the automatic
                                 * calls to &DB::sub */
                              ...
                              if (CxTYPE(cx) == CXt_SUB ||
                                ...
                                const I32 dbcxix =
                                    DPPP_dopoptosub_at(ccstack, cxix - 1);
                                /* We expect that ccstack[dbcxix] is CXt_SUB,
                                   anyway, the field below is defined for any
                                   cx. */
                                /* caller() should not report the automatic
                                 * calls to &DB::sub */
                                if (PL_DBsub && GvCV(PL_DBsub) && dbcxix >= 0 &&
                                    ccstack[dbcxix].blk_sub.cv ==
                                        GvCV(PL_DBsub))
                                  cx = &ccstack[dbcxix];
                              }
...
#define IN_LOCALE_RUNTIME                          \
                              (PL_curcop->op_private & HINT_LOCALE)
#endif
...
#ifndef IN_LOCALE
#define IN_LOCALE                                  \
                              (IN_PERL_COMPILETIME ? IN_LOCALE_COMPILETIME     \
                                                   : IN_LOCALE_RUNTIME)
#endif
...
#define PERL_SCAN_DISALLOW_PREFIX 0x02
                            ...
                            static bool DPPP_(my_grok_numeric_radix)(
                                pTHX_ const char **sp, const char *send);
                            static
#else
                            extern bool DPPP_(my_grok_numeric_radix)(
                                pTHX_ const char **sp, const char *send);
#endif
...
#define Perl_grok_numeric_radix                    \
                              DPPP_(my_grok_numeric_radix)
                                ...
                                bool DPPP_(my_grok_numeric_radix)(
                                    pTHX_ const char **sp, const char *send) {
...
#else
                              /* older perls don't have PL_numeric_radix_sv so
                               * the radix must manually be requested from
                               * locale.h
                               */
#include <locale.h>
                              ...
                              /* always try "." if numeric radix didn't match
                               * because we may have data from different locales
                               * mixed */
                              if (*sp < send && **sp == '.') {
                                ++*sp;
                                return TRUE;
                            ...
                            static int
                                DPPP_(my_grok_number)(pTHX_ const char *pv,
                                                      STRLEN len, UV *valuep);
                            static
#else
                            extern int
                                DPPP_(my_grok_number)(pTHX_ const char *pv,
...
#if defined(NEED_grok_number) ||                   \
                                defined(NEED_grok_number_GLOBAL)
...
#define Perl_grok_number DPPP_(my_grok_number)
                              ...
                              /* next must be digit or the radix separator or
                               * beginning of infinity */
                              if (isDIGIT(*s)) {
                                /* UVs are at least 32 bits, so the first 9
                                   decimal digits cannot overflow.  */
                                UV value = *s - '0';
                                /* This construction seems to be more optimiser
                                   friendly. (without it gcc does the isDIGIT
                                   test and the *s - '0' separately) With it gcc
                                   on arm is managing 6 instructions (6 cycles)
                                   per digit. In theory the optimiser could
                                   deduce how far to unroll the loop before
                              ...
                              skip_value:
                                if (GROK_NUMERIC_RADIX(&s, send)) {
                                  ...
                                  while (s < send &&
                                         isDIGIT(*s)) /* optional digits after
                                                         the radix */
                                    s++;
                                }
                                ...
                                numtype |=
                                    IS_NUMBER_NOT_INT |
                                    IS_NUMBER_IN_UV; /* valuep assigned below */
                                /* no digits before the radix means we need
                                 * digits after it */
                                if (s < send && isDIGIT(*s)) {
                                  do {
                                    s++;
                                ...
                                /* XXX TODO: There are signaling NaNs and quiet
                                 * NaNs. */
                                s++;
                                if (s == send || (*s != 'A' && *s != 'a'))
                                  return 0;
                              ...
                              if (sawinf) {
                                numtype &=
                                    IS_NUMBER_NEG; /* Keep track of sign  */
                                numtype |=
                                ...
                                if (*s == 'e' || *s == 'E') {
                                  /* The only flag we keep is sign.  Blow away
                                   * any "it's UV"  */
                                  numtype &= IS_NUMBER_NEG;
                                  numtype |= IS_NUMBER_NOT_INT;
                                  s++;
                            ...
                            /*
                             * The grok_* routines have been modified to use
                             * warn() instead of Perl_warner(). Also, 'hexdigit'
                             * was the former name of PL_hexdigit, which is why
                             * the stack variable has been renamed to 'xdigit'.
                             */

#ifndef grok_bin
#if defined(NEED_grok_bin)
                            static UV
                                DPPP_(my_grok_bin)(pTHX_ const char *start,
                                                   STRLEN *len_p, I32 *flags,
                                                   NV *result);
                            static
#else
                            extern UV
...
#define Perl_grok_bin DPPP_(my_grok_bin)
                              ...
                              if (!(*flags & PERL_SCAN_DISALLOW_PREFIX)) {
                                /* strip off leading b or 0b.
                                   for compatibility silently suffer "b" and
                                   "0b" as valid binary numbers. */
                                if (len >= 1) {
                                  if (s[0] == 'b') {
                                    s++;
                                    len--;
                              ...
                              for (; len-- && *s; s++) {
                                char bit = *s;
                                if (bit == '0' || bit == '1') {
                                  /* Write it in this wonky order with a goto to
                                     attempt to get the compiler to make the
                                     common case integer-only loop pretty tight.
                                     With gcc seems to be much straighter code
                                     than old scan_bin.  */
                                redo:
                                  if (!overflowed) {
                                    if (value <= max_div_2) {
                                   ...
                                   * mantissa to represent a UV this summing of
                                   ...
                                   * end just multiply value_nv by the right
                                   * amount. */
                                  value_nv += (NV)(bit - '0');
                                  continue;
                                }
                                if (bit == '_' && len && allow_underscores &&
                                    (bit = s[1]) &&
                                    (bit == '0' || bit == '1')) {
                                  --len;
                                  ++s;
                            ...
                            static UV
                                DPPP_(my_grok_hex)(pTHX_ const char *start,
                                                   STRLEN *len_p, I32 *flags,
                                                   NV *result);
                            static
#else
...
#define Perl_grok_hex DPPP_(my_grok_hex)
                              ...
                              if (!(*flags & PERL_SCAN_DISALLOW_PREFIX)) {
                                /* strip off leading x or 0x.
                                   for compatibility silently suffer "x" and
                                   "0x" as valid hex numbers.
                                */
                                if (len >= 1) {
                                  if (s[0] == 'x') {
                                    s++;
                              ...
                              for (; len-- && *s; s++) {
                                ...
                                redo:
                                  if (!overflowed) {
                                    if (value <= max_div_16) {
                                      value = (value << 4) |
                                              ((xdigit - PL_hexdigit) & 15);
                                      continue;
                                    }
                                    warn("Integer overflow in hexadecimal "
                                         "number");
                                    overflowed = TRUE;
                                    value_nv = (NV)value;
                                  }
                                ...
                                if (*s == '_' && len && allow_underscores &&
                                    s[1] &&
                                    (xdigit =
                                         strchr((char *)PL_hexdigit, s[1]))) {
                                  --len;
                                  ++s;
                                  goto redo;
                                }
                                if (!(*flags & PERL_SCAN_SILENT_ILLDIGIT))
                                  warn("Illegal hexadecimal digit '%c' ignored",
                                       *s);
                                break;
                              }
                              ...
                              if ((overflowed && value_nv > 4294967295.0)
                                ...
                                warn("Hexadecimal number > 0xffffffff "
                                     "non-portable");
                              }
                              *len_p = s - start;
                              if (!overflowed) {
                            ...
                            static UV
                                DPPP_(my_grok_oct)(pTHX_ const char *start,
                                                   STRLEN *len_p, I32 *flags,
                                                   NV *result);
                            static
#else
...
#define Perl_grok_oct DPPP_(my_grok_oct)
                              ...
                              for (; len-- && *s; s++) {
                                /* gcc 2.95 optimiser not smart enough to figure
                                   that this subtraction out front allows
                                   slicker code.  */
                                int digit = *s - '0';
                                if (digit >= 0 && digit <= 7) {
                                  /* Write it in this wonky order with a goto to
                                ...
                                if (digit == ('_' - '0') && len &&
                                    allow_underscores && (digit = s[1] - '0') &&
                                    (digit >= 0 && digit <= 7)) {
                                  --len;
                                  ++s;
                                  goto redo;
                                }
                                /* Allow \octal to work the DWIM way (that is,
                                 * stop scanning as soon as non-octal characters
                                 * are seen, complain only iff someone seems to
                                 * want to use the digits eight and nine). */
                                if (digit == 8 || digit == 9) {
                                  if (!(*flags & PERL_SCAN_SILENT_ILLDIGIT))
                                    warn("Illegal octal digit '%c' ignored",
                                         *s);
                                }
                                break;
                              }
                            ...
                            static int DPPP_(my_my_snprintf)(char *buffer,
                                                             const Size_t len,
                                                             const char *format,
                                                             ...);
                            static
#else
                            extern int DPPP_(my_my_snprintf)(char *buffer,
                                                             const Size_t len,
                                                             const char *format,
                                                             ...);
#endif
...
#if defined(NEED_my_snprintf) ||                   \
                                defined(NEED_my_snprintf_GLOBAL)
...
#define Perl_my_snprintf DPPP_(my_my_snprintf)
                            ...
                            static int DPPP_(my_my_sprintf)(char *buffer,
                                                            const char *pat,
                                                            ...);
                            static
#else
                            extern int DPPP_(my_my_sprintf)(char *buffer,
                                                            const char *pat,
                                                            ...);
#endif
...
#define XCPT_RETHROW Siglongjmp(top_env, rEtV)
                            ...
                            static Size_t DPPP_(my_my_strlcat)(char *dst,
                                                               const char *src,
                                                               Size_t size);
                            static
#else
                            extern Size_t DPPP_(my_my_strlcat)(char *dst,
                                                               const char *src,
                                                               Size_t size);
#endif

#if defined(NEED_my_strlcat) ||                    \
                                defined(NEED_my_strlcat_GLOBAL)
...
#define Perl_my_strlcat DPPP_(my_my_strlcat)
                              ...
                              if (size > 0 && used < size - 1) {
                                copy = (length >= size - used) ? size - used - 1
                                                               : length;
                            ...
                            static Size_t DPPP_(my_my_strlcpy)(char *dst,
                                                               const char *src,
                                                               Size_t size);
                            static
#else
                            extern Size_t DPPP_(my_my_strlcpy)(char *dst,
                                                               const char *src,
                                                               Size_t size);
#endif

#if defined(NEED_my_strlcpy) ||                    \
                                defined(NEED_my_strlcpy_GLOBAL)
...
#define UTF8_ALLOW_ANY                             \
                              (UTF8_ALLOW_CONTINUATION |                       \
                               ...
                               UTF8_ALLOW_SHORT | UTF8_ALLOW_LONG |            \
                               UTF8_ALLOW_OVERFLOW)
#endif

#if defined UTF8SKIP

                            /* Don't use official version because it uses MIN,
...
#define D_PPP_BYTE_INFO_BITS                       \
                              6 /* 6 bits meaningful in continuation bytes */
#ifdef QUADKIND
#define D_PPP_UVCHR_SKIP_UPPER(c)                  \
                              (WIDEST_UTYPE)(c) <                              \
                                      (((WIDEST_UTYPE)1)                       \
                                       << (6 * D_PPP_BYTE_INFO_BITS))          \
                                  ? 7                                          \
                                  : 13
#else
#define D_PPP_UVCHR_SKIP_UPPER(c)                  \
...
#else
#define D_PPP_BYTE_INFO_BITS                       \
                              5 /* EBCDIC has only 5 meaningful bits */

                            /* In the releases this is backported to, UTF-EBCDIC
                             * had a max of 2**31-1 */
...
#define UVCHR_SKIP(c)                              \
                              ...
                              : (WIDEST_UTYPE)(c) <                            \
                                      (32 * (1U << (D_PPP_BYTE_INFO_BITS)))    \
                                  ? 2                                          \
                              : (WIDEST_UTYPE)(c) <                            \
                                      (16 *                                    \
                                       (1U << (2 * D_PPP_BYTE_INFO_BITS)))     \
                                  ? 3                                          \
                              : (WIDEST_UTYPE)(c) <                            \
                                      (8 * (1U << (3 * D_PPP_BYTE_INFO_BITS))) \
                                  ? 4                                          \
                              : (WIDEST_UTYPE)(c) <                            \
                                      (4 * (1U << (4 * D_PPP_BYTE_INFO_BITS))) \
                                  ? 5                                          \
                              : (WIDEST_UTYPE)(c) <                            \
                                      (2 * (1U << (5 * D_PPP_BYTE_INFO_BITS))) \
                                  ? 6                                          \
...
#define is_invariant_string(s, l)                  \
                              is_ascii_string(s, l)
#endif

#ifndef is_utf8_invariant_string
#define is_utf8_invariant_string(s, l)             \
                              is_ascii_string(s, l)
#endif

                            /* Hint: is_ascii_string, is_invariant_string
                               is_utf8_invariant_string() does the same thing
                               and is preferred because its name is more
                               accurate as to what it does */
#endif

#ifdef ibcmp_utf8
#ifndef foldEQ_utf8
#define foldEQ_utf8(s1, pe1, l1, u1, s2, pe2, l2,  \
                                                u2)                            \
                              cBOOL(!ibcmp_utf8(s1, pe1, l1, u1, s2, pe2, l2,  \
                                                u2))
#endif
...
#ifndef REPLACEMENT_CHARACTER_UTF8
#define REPLACEMENT_CHARACTER_UTF8                 \
                              "\xDD\x73\x73\x71"
#endif
...
#error Unknown character set
                            ...
                            /* Versions prior to this accepted things that are
                             * now considered malformations, and didn't return
                             * -1 on error with warnings enabled
                             * */
#undef utf8_to_uvchr_buf
#endif

                            /* This implementation brings modern, generally more
                             * restricted standards to utf8_to_uvchr_buf.  Some
                             * of these are security related, and clearly must
                             * be done.  But its arguable that the others need
                             * not, and hence should not. The reason they're
                             * here is that a module that intends to play with
                             * the latest perls should be able to work the same
                             * in all releases.  An example is that perl no
                             * longer accepts any UV for a code point, but
                             * limits them to IV_MAX or below.  This is for
                             * future internal use of the larger code points. If
                             * it turns out that some of these changes are
                             * breaking code that isn't intended to work with
                             * modern perls, the tighter restrictions could be
                             * relaxed.  khw thinks this is unlikely, but has
                             * been wrong in the past. */

                            /* 5.6.0 is the first release with UTF-8, and we
                             * don't implement this function there due to its
                             * likely lack of still being in use, and the
                             * underlying implementation is very different from
                             * later ones, without the later safeguards, so
                             * would require extra work to deal with */
#if (PERL_BCDVERSION >= 0x5006001) &&              \
                                !defined(utf8_to_uvchr_buf)
                            /* Choose which underlying implementation to use. At
                             * least one must be present or the perl is too
                             * early to handle this function */
#if defined(utf8n_to_uvchr) ||                     \
                                defined(utf8_to_uvchr) || defined(utf8_to_uv)
#if defined(utf8n_to_uvchr) /* This is the         \
                                                           preferred           \
                                                           implementation */
#define D_PPP_utf8_to_uvchr_buf_callee             \
                              utf8n_to_uvchr
#elif /* Must be at least 5.6.1 from #if above;    \
                                     If have both regular and _simple, regular \
                                     has all args */                           \
                                defined(utf8_to_uv) &&                         \
                                defined(utf8_to_uv_simple)
#define D_PPP_utf8_to_uvchr_buf_callee utf8_to_uv
#elif defined(                                     \
                                utf8_to_uvchr) /* The below won't work well on \
                                                  error input */
#define D_PPP_utf8_to_uvchr_buf_callee(            \
                                s, curlen, retlen, flags)                      \
...
#define D_PPP_utf8_to_uvchr_buf_callee(            \
                            ...
                            static UV
                                DPPP_(my_utf8_to_uvchr_buf)(pTHX_ const U8 *s,
                                                            const U8 *send,
                                                            STRLEN *retlen);
                            static
#else
...
#if defined(NEED_utf8_to_uvchr_buf) ||             \
                                defined(NEED_utf8_to_uvchr_buf_GLOBAL)

#ifdef utf8_to_uvchr_buf
#undef utf8_to_uvchr_buf
#endif
#define utf8_to_uvchr_buf(a, b, c)                 \
                              DPPP_(my_utf8_to_uvchr_buf)(aTHX_ a, b, c)
#define Perl_utf8_to_uvchr_buf                     \
                              DPPP_(my_utf8_to_uvchr_buf)
                              ...
                              if (send > s) {
                                ...
                                assert(0); /* Modern perls die under this
                                              circumstance */
                                curlen = 0;
                                if (!do_warnings) { /* Handle empty here if no
                                                       warnings needed */
                                  if (retlen)
                                    *retlen = 0;
                              ...
                              /* Perl did not properly detect overflow for much
                               * of its history on non-EBCDIC platforms, often
                               * returning an overlong value which may or may
                               * not have been tolerated in the call.  Also,
                               * earlier versions, when they did detect
                               * overflow, may have disallowed it completely.
                               * Modern ones can replace it with the REPLACEMENT
                               * CHARACTER, depending on calling parameters.
                               * Therefore detect it ourselves in  releases it
                               * was problematic in. */

                              if (curlen > 0 && UNLIKELY(*s >= 0xFE)) {

                                /* First, on a 32-bit machine the first byte
                                 * being at least \xFE automatically is
                                 * overflow, as it indicates something requiring
                                 * more than 31 bits */
                                if (sizeof(ret) < 8) {
                                  overflows = 1;
                                  overflow_length = 7;
                                  ...
                                  const U8 highest[] = /* 2*63-1 */
                                      "\xFF\x80\x87\xBF\xBF\xBF\xBF\xBF\xBF\xBF"
                                      "\xBF\xBF\xBF";
                                  const U8 *cur_h = highest;

                                  for (cur_s = s; cur_s < send;
                                       cur_s++, cur_h++) {
                                    if (UNLIKELY(*cur_s == *cur_h)) {
                                      continue;
                                    }

                                    /* If this byte is larger than the
                                     * corresponding highest UTF-8 byte, the
                                     * sequence overflows; otherwise the byte is
                                     * less than (as we handled the equality
                                     * case above), and so the sequence doesn't
                                     * overflow */
                                    overflows = *cur_s > *cur_h;
                                    break;
                                  }

                                  /* Here, either we set the bool and broke out
                                   * of the loop, or got to the end and all
                                   * bytes are the same which indicates it
                                   * doesn't overflow.  If it did overflow, it
                                   * would be this number of bytes
                                   * */
                                  overflow_length = 13;
                                }
                              }
                              ...
                              if (UNLIKELY(overflows)) {
                                ...
                                /* Here, we are either in a release that
                                 * properly detects overflow, or we have checked
                                 * for overflow and the next statement is
                                 * executing as part of the above conditional
                                 * where we know we don't have overflow.
                                 *
                                 * The modern versions allow anything that
                                 * evaluates to a legal UV, but not overlongs
                                 * nor an empty input */
...
#if (PERL_BCDVERSION >= 0x5026000) &&              \
                                (PERL_BCDVERSION < 0x5028000)

                              /* But actually, more modern versions restrict the
                               * UV to being no more than what an IV can hold,
                               * so it could still have gotten it wrong about
                               * overflowing. */
                              ...
                              if (UNLIKELY(overflows)) {
                                if (!do_warnings) {
                                  ...
                                  /* We use the error message in use
                                   * from 5.8-5.26 */
                                  Perl_warner(
                                      aTHX_ packWARN(WARN_UTF8),
                                      "Malformed UTF-8 character (overflow at "
                                      "0x%" UVxf
                              ...
                              /* Here, did not overflow, but if it failed for
                               * some other reason, and warnings are off, to
                               * emulate the behavior of the real
                               * utf8_to_uvchr(), try again, allowing anything.
                               * (Note a return of 0 is ok if the input was
                               * '\0') */
                              if (UNLIKELY(ret == 0 &&
                                           (curlen == 0 || *s != '\0'))) {

                                /* If curlen is 0, we already handled the case
                                 * where warnings are disabled, so this 'if'
                                 * will be true, and so later on, we know that
                                 * 's' is dereferencible */
                                if (do_warnings) {
                                  *retlen = (STRLEN)-1;
                                } else {
                                  ...
                                  /* Override with the REPLACEMENT character, as
                                   * that is what the modern version of this
                                   * function returns */
                                   ...
                                   * necessarily return the proper length.  It
                                   * should not extend past the end of string,
                                   * nor past what the first byte indicates the
                                   * length is, nor past the continuation
                                   * characters */
                                  if (retlen && *retlen >= 0) {
                                    unsigned int i = 1;
...
#undef utf8_to_uvchr /* Always redefine this       \
                                                    unsafe function so that it \
                                                    refuses to read past a                                                                \
                                                    NUL, making it much less   \
                                                    likely to read off the end                                                                   \
                                                    of the buffer.  A NUL      \
                                                    indicates the start of the                                                                        \
                                                    next character anyway.  If \
                                                    the input isn't            \
                                                    NUL-terminated, the        \
                                                    function remains unsafe,   \
...
#define utf8_to_uvchr(s, lp)                       \
                              ((*(s) == '\0')                                  \
                                   ? utf8_to_uvchr_buf(                        \
                                         s, ((s) + 1),                         \
                                         lp) /* Handle single NUL specially */ \
                                   : utf8_to_uvchr_buf(                        \
                                         s,                                    \
                                         (s) + my_strnlen((char *)(s),         \
                                                          UTF8SKIP(s)),        \
                            ...
                            /* Hint: utf8_to_uvchr
                                Use utf8_to_uvchr_buf() instead.  But ONLY if
                               you KNOW the upper bound of the input string (not
                               resorting to using UTF8SKIP, etc., to infer it).
                                The backported utf8_to_uvchr() will do a better
                               job to prevent most cases of trying to read
                               beyond the end of the buffer */

                            /* Replace utf8_to_uvchr with utf8_to_uvchr_buf */

#ifdef SV_NOSTEAL
                            /* Older Perl versions have broken sv_len_utf8()
                             * when passed sv does not have SVf_UTF8 flag set */
                            /* Also note that SvGETMAGIC() may change presence
                             * of SVf_UTF8 flag */
#if (PERL_BCDVERSION < 0x5017005)
#undef sv_len_utf8
#if defined(__GNUC__) &&                           \
...
#define sv_len_utf8_nomg(sv)                       \
                              (PL_Sv = (sv),                                   \
                               (SvUTF8(PL_Sv)                                  \
                                    ? Perl_sv_len_utf8(                        \
                                          aTHX_(!SvGMAGICAL(PL_Sv)             \
                                                    ? PL_Sv                    \
                                                    : sv_mortalcopy_flags(     \
                                                          PL_Sv, SV_NOSTEAL))) \
                                    : (SvPV_nomg(PL_Sv, PL_na), PL_na)))
#define sv_len_utf8(sv)                            \
                              (PL_Sv = (sv), SvGETMAGIC(PL_Sv),                \
...
#define PERL_PV_PRETTY_REGPROP                     \
                            ...
                            /* Hint: pv_escape
                             * Note that unicode functionality is only
                             * backported to those perl versions that support
                             * it. For older perl versions, the implementation
                             * will fall back to bytes.
                             */

#ifndef pv_escape
#if defined(NEED_pv_escape)
                            static char *DPPP_(my_pv_escape)(
                                pTHX_ SV *dsv, char const *const str,
                                const STRLEN count, const STRLEN max,
                                STRLEN *const escaped, const U32 flags);
                            static
#else
                            extern char *DPPP_(my_pv_escape)(
                                pTHX_ SV *dsv, char const *const str,
                                const STRLEN count, const STRLEN max,
                                STRLEN *const escaped, const U32 flags);
#endif
...
#define Perl_pv_escape DPPP_(my_pv_escape)
                                ...
                                char *DPPP_(my_pv_escape)(pTHX_ SV *dsv,
                                                          ...
                                                          const U32 flags) {
                              const char esc =
                                  flags & PERL_PV_ESCAPE_RE ? '%' : '\\';
                              const char dq =
                                  flags & PERL_PV_ESCAPE_QUOTE ? '"' : esc;
                              ...
                              for (; pv < end && (!max || wrote < max);
                                   pv += readsize) {
                                const UV u =
#if defined(is_utf8_string) &&                     \
                                defined(utf8_to_uvchr_buf)
                                    isuni ? utf8_to_uvchr_buf((U8 *)pv, end,
                                                              &readsize)
                                          :
#endif
                                          (U8)*pv;
                                ...
                                if (u > 255 || (flags & PERL_PV_ESCAPE_ALL)) {
                                  if (flags & PERL_PV_ESCAPE_FIRSTCHAR)
                                    ...
                                    chsize =
                                        my_snprintf(octbuf, sizeof octbuf,
                                                    "%cx{%" UVxf "}", esc, u);
                            ...
                            static char *DPPP_(my_pv_pretty)(
                                pTHX_ SV *dsv, char const *const str,
                                const STRLEN count, const STRLEN max,
                            ...
                            static
#else
                            extern char *DPPP_(my_pv_pretty)(
                                pTHX_ SV *dsv, char const *const str,
                                const STRLEN count, const STRLEN max,
                                char const *const start_color,
                                char const *const end_color, const U32 flags);
...
#define Perl_pv_pretty DPPP_(my_pv_pretty)
                              ...
                              pv_escape(dsv, str, count, max, &escaped,
                              ...
                              if ((flags & PERL_PV_PRETTY_ELLIPSES) &&
                                  escaped < count)
                                sv_catpvs(dsv, "...");
                            ...
                            static char *DPPP_(my_pv_display)(pTHX_ SV *dsv,
                                                              const char *pv,
                                                              STRLEN cur,
                                                              STRLEN len,
                                                              STRLEN pvlim);
                            static
#else
                            extern char *DPPP_(my_pv_display)(pTHX_ SV *dsv,
                                                              const char *pv,
                                                              STRLEN cur,
                                                              STRLEN len,
                                                              STRLEN pvlim);
...
#if defined(NEED_pv_display) ||                    \
                                defined(NEED_pv_display_GLOBAL)
...
#define Perl_pv_display DPPP_(my_pv_display)
                                ...
                                char *
                                DPPP_(my_pv_display)(pTHX_ SV *dsv,
                                                     const char *pv, STRLEN cur,
...
#define switch_to_global_locale()
                            ...
                            /* Originally, this didn't return a value, but in
                             * perls like that, the value should always be TRUE.
                             * Add a return to Perl_sync_locale() when it's
                             * available.  And actually do a sync when its not,
                             * if locales are available on this system. */
#ifdef sync_locale
...
#define sync_locale()                              \
                              (new_ctype(setlocale(LC_CTYPE, NULL)), 1)
#else
#undef sync_locale
#endif
#endif
