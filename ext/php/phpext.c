...
static zend_class_entry *php_syck_get_exception_base(void) {
  zend_string *key =
      zend_string_init(PHP_SYCK_EXCEPTION_PARENT_LC,
...
static void psex_add_output(php_syck_emitter_xtra *px, const char *data,
                            size_t len) {
...
static int psex_determine_array_type(HashTable *ht) {
  ...
  ZEND_HASH_FOREACH_KEY(ht, num, key) {
    if (key)
      return 1; /* string key  -> map */
    if (num != idx)
      return 1; /* gap / reorder-> map */
    idx++;
  }
...
enum st_retval my_cleaner(SHIM(const char *key), void *value,
                          SHIM(void *smth)) {
  zval *ptr = (zval *)value;
...
void php_syck_ehandler(SyckParser *p, const char *str) {
  ...
  if (NULL == p->bonus) {
    ...
    zend_object *exc = zend_throw_exception_ex(
        syck_exception_entry, 0, "%s on line %ld, col %ld: '%s'", str,
        p->linect + 1, (long)(p->cursor - p->lineptr), p->lineptr);
...
static void php_syck_emitter_handler(SyckEmitter *e, st_data_t id) {
...
#endif
    syck_emit_scalar(e, "int", scalar_none, 0, 0, 0, ZSTR_VAL(str.s),
                     ZSTR_LEN(str.s));
    smart_str_free(&str);
    break;
  }
  case IS_DOUBLE: {
    ...
    smart_str_append_double(&str, Z_DVAL_P(zv), 10, 0);
#endif
    syck_emit_scalar(e, "float", scalar_none, 0, 0, 0, ZSTR_VAL(str.s),
                     ZSTR_LEN(str.s));
    smart_str_free(&str);
    break;
  }
  case IS_STRING:
    syck_emit_scalar(e, "str", scalar_2quote, 0, 0, 0, Z_STRVAL_P(zv),
                     Z_STRLEN_P(zv));
    break;
...
static const zend_function_entry syck_functions[] = {
    PHP_FE(syck_load, arginfo_syck_load) PHP_FE(syck_dump, arginfo_syck_dump)
        PHP_FE_END};

static const zend_module_dep syck_deps[] = {ZEND_MOD_REQUIRED(
    "spl") ZEND_MOD_REQUIRED("hash") ZEND_MOD_REQUIRED("date") ZEND_MOD_END};

zend_module_entry syck_module_entry = {STANDARD_MODULE_HEADER_EX,
                                       NULL,
                                       syck_deps,
                                       "syck",
                                       syck_functions,
