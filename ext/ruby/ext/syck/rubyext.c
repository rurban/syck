 ...
 */
static ID s_new, s_utc, s_at, s_to_f, s_to_i, s_read, s_binmode, s_call, s_cmp,
    s_transfer, s_update, s_dup, s_haskey, s_match, s_keys, s_unpack, s_tr_bang,
    s_default_set, s_tag_read_class, s_tag_subclasses, s_resolver, s_push,
    s_emitter, s_level, s_detect_implicit, s_node_import, s_out, s_input,
    s_intern, s_transform, s_yaml_new, s_yaml_initialize, s_node_export,
    s_to_yaml, s_write, s_set_resolver, s_each;
static ID s_tags, s_kind, s_name, s_options, s_type_id, s_type_id_set, s_style,
    s_style_set, s_value, s_value_set, s_parse;
static VALUE sym_model, sym_generic, sym_input, sym_bytecode;
static VALUE sym_scalar, sym_seq, sym_map;
static VALUE sym_1quote, sym_2quote, sym_fold, sym_literal, sym_plain,
    sym_inline;
static VALUE cDate, cNode, cMap, cSeq, cScalar, cOut, cParser, cResolver,
    cPrivateType, cDomainType, cYObject, cBadAlias, cDefaultKey, cMergeKey,
    cEmitter, cDateTime;
...
struct parser_xtra {
  VALUE data; /* Borrowed this idea from marshal.c to fix [ruby-core:8067]
                 problem */
  VALUE proc;
  VALUE resolver;
  int taint;
...
rb_syck_compile(VALUE self, VALUE port) {
  ...
  if (!syck_lookup_sym(parser, oid, (char **)&data)) {
...
mktime_do(VALUE varg) {
  ...
  /* Time Zone*/
  while (len > ptr - str && *ptr != 'Z' && *ptr != '+' && *ptr != '-' &&
         *ptr != '\0')
    ptr++;
  if (len > ptr - str && (*ptr == '-' || *ptr == '+')) {
    ...
    /* Make UTC time*/
    return rb_funcall(rb_cTime, s_utc, 7, year, mon, day, hour, min, sec,
                      LONG2NUM(usec));
  }
}
...
rb_syck_mktime(const char *str, long len) {
  ...
  a.str = str;
  a.len = len;
  return rb_rescue2(mktime_do, (VALUE)&a, mktime_r, (VALUE)&a, rb_eArgError,
                    NULL);
}
...
int yaml_org_handler(SyckNode *n, VALUE *ref) {
  ...
  case syck_str_kind:
    ...
    } else if (n->data.str->style == scalar_plain && n->data.str->len > 1 &&
...
void rb_syck_err_handler(SyckParser *p, const char *msg) {
  ...
  endl[0] = '\0';
  rb_raise(rb_eArgError, "%s on line %d, col %" PRIdPTRDIFF ": `%s'", msg,
           p->linect, p->cursor - p->lineptr, p->lineptr);
}

/*
 * provide bad anchor object to the parser.
 */
SyckNode *rb_syck_bad_anchor_handler(SyckParser *p, const char *a) {
  VALUE anchor_name = rb_str_new2(a);
  SyckNode *badanc = syck_new_map(rb_str_new2("name"), anchor_name);
  badanc->type_id =
      syck_strndup("tag:ruby.yaml.org,2002:object:YAML::Syck::BadAlias", 53);
  return badanc;
}
...
int syck_st_mark_nodes(st_data_t key, st_data_t n, st_data_t arg) {
  if (n != 1UL)
 ...
 * mark parser nodes
 */
static void syck_mark_parser(void *_parser) {
 ...
 */
void rb_syck_free_parser(void *_p) {
 ...
 */
VALUE
syck_resolver_detect_implicit(VALUE self, VALUE val) { return rb_str_new2(""); }
...
syck_const_find(VALUE const_name) {
  ...
  for (i = 0; i < RARRAY_LEN(tparts); i++) {
    ...
    if (!rb_const_defined(tclass, tpart))
      return Qnil;
  ...
  if (!(NIL_P(type) || RSTRING_LEN(StringValue(type)) == 0)) {
    ...
    if (NIL_P(target_class)) {
      ...
      while (RARRAY_LEN(parts) > 1) {
        ...
        if (!NIL_P(target_class)) {
          ...
          if (RARRAY_LEN(subclass_parts) > 0 &&
              rb_respond_to(target_class, s_tag_subclasses) &&
    ...
    if (rb_respond_to(target_class, s_call)) {
      ...
      if (rb_respond_to(target_class, s_yaml_new)) {
        ...
        {
          obj = rb_str2inum(val,
                            10); /* for yaml dumped by 1.8.3 [ruby-core:6159] */
        } else {
...
syck_genericresolver_node_import(VALUE self, VALUE node) {
  ...
  case syck_map_kind:
    ...
    for (i = 0; i < syck_map_count(n); i++) {
      rb_hash_aset(v, syck_map_read(n, map_key, i),
                   syck_map_read(n, map_value, i));
    }
...
syck_node_transform(VALUE self) {
  ...
  case syck_map_kind: {
    ...
    for (i = 0; i < orig_n->data.pairs->idx; i++) {
      syck_map_add(
          n, rb_funcall(syck_map_read(orig_n, map_key, i), s_transform, 0),
  ...
  case syck_str_kind:
    DATA_PTR(t) = n = syck_new_str2(
        orig_n->data.str->ptr, orig_n->data.str->len, orig_n->data.str->style);
    break;
  }
...
void rb_syck_emitter_handler(SyckEmitter *e, st_data_t data) {
  ...
  case syck_str_kind: {
    syck_emit_scalar(e, n->type_id, n->data.str->style, 0, 0, 0,
 ...
 */
void rb_syck_output_handler(SyckEmitter *emitter, const char *str, long len) {
...
static void rb_syck_free_node(void *node) { syck_free_node((SyckNode *)node); }

/*
 * Mark emitter values.
 */
static void syck_mark_emitter(void *_emitter) {
 ...
 */
void rb_syck_free_emitter(void *_e) {
...
void Init_syck() {
  ...
  rb_define_method(cResolver, "detect_implicit", syck_resolver_detect_implicit,
                   1);
  ...
  rb_define_singleton_method(oDefaultResolver, "node_import",
                             syck_defaultresolver_node_import, 1);
  rb_define_singleton_method(oDefaultResolver, "detect_implicit",
                             syck_defaultresolver_detect_implicit, 1);
  ...
  rb_define_singleton_method(oGenericResolver, "node_import",
                             syck_genericresolver_node_import, 1);
  ...
  rb_include_module(cBadAlias,
                    rb_const_get(rb_cObject, rb_intern("Comparable")));
