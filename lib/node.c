 ...
 */
__attribute__malloc__ static SyckNode *
...
void syck_free_node(SyckNode **np) {
  SyckNode *n = *np;
  DPRINTF((stderr, "DEBUG %s  %p (%s)\n", __FUNCTION__, n, syck_node_kind(n)));
  syck_free_members(n);
...
void syck_free_name(char *name) {
  DPRINTF((stderr, "DEBUG %s %s\n", __FUNCTION__, name));
  S_FREE(name);
}

__attribute__malloc__ SyckNode *syck_alloc_map(void) {
  SyckNode *n;
  struct SyckMap *m;
...
__attribute__malloc__ SyckNode *syck_alloc_str(void) {
  SyckNode *n;
  struct SyckStr *s;
...
__attribute__malloc__ SyckNode *syck_new_str2(char *str, long len,
...
void syck_replace_str2(SyckNode *n, char *str, long len,
                       enum scalar_style style) {
...
__attribute__warn_unused_result__ __attribute__returns_nonnull__ const char *
