...
static __attribute__unused__ __attribute__noreturn__ void
syck_vmefail(size_t size) {
...
static void syck_parser_reset_levels(SyckParser *p) {
  while (p->lvl_idx > 1) {
...
static void syck_parser_reset_cursor(SyckParser *p) {
  if (p->buffer == NULL) {
 ...
 * Allocate the parser
 */
__attribute__malloc__ SyckParser *syck_new_parser(void) {
  SyckParser *p;
...
int syck_lookup_sym(SyckParser *p, SYMID id, char **data) {
  if (p->syms == NULL)
    return 0;
...
int syck_st_free_nodes(st_data_t key, st_data_t record, st_data_t arg)
#else
enum st_retval syck_st_free_nodes(SHIM(const char *key), void *record,
                                  SHIM(void *arg))
#endif
{
...
static void syck_st_free(SyckParser *p) {
...
void syck_parser_str_auto(SyckParser *p, const char *ptr, SyckIoStrRead read) {
...
void syck_parser_add_level(SyckParser *p, int len,
  ...
  p->levels[p->lvl_idx].domain =
      syck_strndup(p->levels[p->lvl_idx - 1].domain,
                   strlen(p->levels[p->lvl_idx - 1].domain));
...
static long syck_move_tokens(SyckParser *p) {
  long count, skip;
  assert(p->buffer);
...
static void syck_check_limit(SyckParser *p, long len) {
  if (p->cursor == NULL) {
    p->cursor = p->buffer;
    p->lineptr = p->buffer;
...
int syck_str_is_unquotable_integer(const char *str, long len) {
  ...
  if (!str)
    return 0; /* Don't parse null strings */
  if (len < 1)
    return 0; /* empty strings can't be numbers */
  if (len > 9)
    return 0; /* Ints larger than 9 digits (32bit) might not portable. Force a
                 string. */

  if (str[0] == '0' && len == 1)
    return 1; /* 0 is unquoted. */
  if (str[0] == '-') {
    str++;
    len--;
  } /* supress the leading '-' sign if detected for testing purposes only. */
  if (str[0] == '0')
    return 0; /* Octals need to be quoted or you lose data converting them to an
                 integer. This also accidentally blocks -0 which probably needs
                 to be quoted. */
