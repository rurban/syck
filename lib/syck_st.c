...
static int strhash(const void *arg) {
...
static int new_size(int size) {
  ...
  for (i = 0, newsize = MINSIZE; i < sizeof(primes) / sizeof(primes[0]);
       i++, newsize <<= 1) {
    if (newsize > size)
      return primes[i];
  }
...
int st_lookup(st_table *table, const char *key, const void **value) {
...
int st_delete(st_table *table, const void **key, const void **value) {
  unsigned int hash_val;
  st_table_entry *tmp;
  st_table_entry *ptr;
...
int st_delete_safe(st_table *table, const void **key, const void **value,
                   char *never) {
  unsigned int hash_val;
  st_table_entry *ptr;
...
static enum st_retval delete_never(SHIM(const char *key), void *value,
                                   void *never) {
  UNUSED(key);
  if (value == never)
    return ST_DELETE;
...
static void delete_anchor_duplicates(st_table *table, void *record) {
  ...
  for (i = 0; i < table->num_bins; i++) {
    ...
    for (ptr = table->bins[i]; ptr != NULL;) {
      ...
      if (func == syck_st_free_nodes && ptr->record) {
        DPRINTF(
            (stderr, "DEBUG %s unset ptr->key %p\n", __FUNCTION__, ptr->key));
