...
do {\
    ...
    DPRINTF ((stderr, "DEBUG: %s new table entry %p: '%lx' %p\n", __FUNCTION__, \
              entry, (uintptr_t)key, value)); \
    \
    entry->hash = hash_val;\
    entry->key = key;\
    entry->record = value;\
...
st_insert(st_table *table, const char *key, const void *value)
{
	...
	ptr->record = value;
        DPRINTF ((stderr, "DEBUG: %s change table entry %p: '%lx' %p\n", __FUNCTION__,
                  ptr, (uintptr_t)key, value));
	return 1;
    }
}
